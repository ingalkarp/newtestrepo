...
export class ShoppingcartComponent implements OnInit, OnDestroy {
  ...
  deleteCartItem(itemId: number) {
    this.cartService.removeCartItems(this.userId, itemId)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(
        result => {
  ...
  addToCart(itemId: number) {
    this.cartService.addProductToCart(this.userId, itemId)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(
        result => {
          this.subscriptionService.cartItemcount$.next(result);
          ...
          console.log('Error ocurred while add to Cart : ', error);
        });
  }

  deleteOneCartItem(itemId: number) {
    this.cartService.deleteOneCartItem(this.userId, itemId)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(
        result => {
          this.subscriptionService.cartItemcount$.next(result);
